...
def render_3d(
    ...
    if cfg.SHOW_AXES:
        plotter.add_axes(
            xlabel="X (L->R)", ylabel="Y (P->A)", zlabel="Z (I->S)",
            line_width=3,
            viewport=(0.0, 0.0, 0.15, 0.15),   # pin to bottom-left, no clash
        )

    # Legend → lower-right, compact height
    plotter.add_legend(
        size=(0.32, 0.10),
        loc="lower right",
        face="circle",
    ...
    # Title → upper-left
    plotter.add_text(
        f"Liver Segmentation  –  {case_name}",
        position="upper_left",
        font_size=11,
        color="black",
    )

    # Dice text → upper-right using normalised coords so it never clips
    if dice_scores:
        overlay = "\n".join([
            "Dice Scores",
        ...
        plotter.add_text(
            overlay,
            position=(0.70, 0.80),   # normalised: clear of both legend and edges
            font_size=10,
            color="black",
        )
...
def main():
    ...
    if gt_path is not None:
        ...
        if gt is not None:
            dice_scores = compute_dice_scores(pred, gt, cfg)
